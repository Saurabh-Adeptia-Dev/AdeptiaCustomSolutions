...
public class AdeptiaFileRouter {
    ...
    private static List<TransferResult> transferParallel(
        ...
        try {
            ...
            // Shutdown was interrupted -- force stop and restore interrupt flag.
            // Any in-flight threads close their own connections via try-with-resources.
            LOG.warn("Executor awaitTermination interrupted in transferParallel. Forcing shutdown.");
            executor.shutdownNow();
            Thread.currentThread().interrupt();
        }
    ...
    private static List<TransferResult> transferDirectParallel(
        ...
        try {
            ...
            // Shutdown was interrupted -- force stop and restore interrupt flag.
            // Any in-flight threads close their own connections via try-with-resources.
            LOG.warn("Executor awaitTermination interrupted in transferDirectParallel. Forcing shutdown.");
            executor.shutdownNow();
            Thread.currentThread().interrupt();
        }
        return results;
