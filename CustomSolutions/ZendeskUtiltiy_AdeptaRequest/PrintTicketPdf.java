...
import org.jsoup.safety.Safelist; // sanitize html_body
...
import java.text.Normalizer; // unicode normalization
...
//    private static final String TEMPLATE_HTML_RES = TemplateRepository.ticketHtml();
//    private static final String TEMPLATE_CSS_RES  = TemplateRepository.ticketCSS();
    ...
    public static void printTicket(List<Long> ticketIds, String configFile) throws Exception {
        ...
        // Safer for CLI tools (avoid Windows proxy auto-detection issues)
        ...
        // ✅ Let JDK manage HttpClient threads
        ...
        logger.info("[env] user.dir={}", Paths.get("").toAbsolutePath());
        logger.info("[env] outDir={} writable={}", OUT_DIR_PATH, Files.isWritable(OUT_DIR_PATH));
        ...
        for (long ticketId : ticketIds) {
            pool.submit(() -> {
                try {
                    ...
                    logger.error("FATAL error for ticket {}", ticketId, t);
                }
            });
        }
    ...
    private static void processOne(long ticketId,
                                   Map<Long, UserDisplay> userCache,
                                   Map<Long, String> groupCache) throws Exception {

        logger.info("[ticket:{}] START", ticketId);

        // Resolve ticket directory (DO NOT create yet)
        Path ticketDir = OUT_DIR.toPath().resolve(Long.toString(ticketId));
        Path attachmentsDir = ticketDir.resolve("attachments");

        // -------------------- FETCH CORE DATA --------------------

        logger.info("[ticket:{}] Fetching ticket JSON", ticketId);
        JsonObject ticket = fetchTicket(ticketId);

        logger.info("[ticket:{}] Fetching comments", ticketId);
        List<JsonObject> comments = fetchAllComments(ticketId);

        // -------------------- DOWNLOAD ATTACHMENTS --------------------

        boolean attachmentsWritten = false;
        ...
        for (JsonObject comment : comments) {
            ...
            for (JsonElement a : atts) {
                ...
                try {
                    logger.info("[ticket:{}] Downloading attachment {}", ticketId, fileName);
                    saveAttachment(HTTP, AUTH, URI.create(fileUrl), attachmentsDir, fileName);
                    attachmentsWritten = true;
                } catch (Exception e) {
                    logger.error("[ticket:{}] Attachment failed ({}): {}",
                            ticketId, fileName, e.getMessage(), e);
                    appendErrorCsv(ticketId, "attachment:" + fileName, e.getMessage());
                }
            }
        }

        if (attachmentsWritten) {
            logger.info("[ticket:{}] Attachments downloaded", ticketId);
        }

        // -------------------- RESOLVE USERS --------------------

        logger.info("[ticket:{}] Resolving users", ticketId);
        ...
        logger.info("[ticket:{}] Building HTML", ticketId);
        ...
        logger.info("[ticket:{}] Writing PDF", ticketId);

        // Create ticket directory ONLY when output is guaranteed
        Files.createDirectories(ticketDir);

        File pdf = ticketDir
                .resolve("ticket-" + tNode.path("id").asText("") + ".pdf")
                .toFile();

        htmlToPdf(html, pdf);

        logger.info("[ticket:{}] DONE (PDF + {} attachments)",
                ticketId,
                attachmentsWritten ? "some" : "no");
    }


    private static void initTemplateResources() {
        TEMPLATE_HTML_RAW = TemplateRepository.ticketHtml();
        TEMPLATE_CSS_RAW  = TemplateRepository.ticketCSS();
    }
    ...
    private static JsonNode getJson(String url) throws Exception {
        ...
        if (res.statusCode() != 200) {
            ...
            logger.error("GET {} -> HTTP {} {}", url, res.statusCode(), body);
            throw new IOException("Zendesk API returned " + res.statusCode());
        }
    ...
    private static HttpResponse<String> sendGetWithRetry(String url, int attempts) throws Exception {
        ...
        while (tries < attempts) {
            tries++;
            try {
                logger.info("[http] GET attempt {}/{} {}", tries, attempts, url);
                ...
                if (code == 429 || (code >= 500 && code < 600)) {
                    ...
                    logger.warn("[http] {} -> HTTP {} (retrying in {} ms)",
                            url, code, Math.max(300L, sleep + jitter));
                ...
                logger.warn("[http] {} -> IO error: {} (retrying in {} ms)",
                        url, ioe.getMessage(), Math.max(200L, sleep + jitter));
                ...
                logger.error("[http] Interrupted while calling {}", url);
                break;
            }
        }
    ...
    private static List<JsonObject> fetchAllComments(long ticketId) throws Exception {
        ...
        while (url != null) {
            ...
            if (res.statusCode() != 200) {
                throw new IOException("comments HTTP " + res.statusCode() +
                        " for " + url + " :: " + res.body());
            }
    ...
    private static void writeErrorsCsv(File file, List<ErrorRow> rows) {
        ...
        try (PrintWriter pw = new PrintWriter(file, StandardCharsets.UTF_8)) {
            pw.println("ticket_id,phase,message");
            rows.stream()
                    .sorted(Comparator.comparingLong(r -> r.ticketId))
                    .forEach(r -> pw.println(r.ticketId + "," + csv(r.phase) + "," + csv(r.message)));
        } catch (IOException ioe) {
    ...
    private static final class ErrorRow {
        ...
        ErrorRow(long ticketId, String phase, String message) {
